...
#### 3. Файлы проекта
При правильном создании проекта в файлах должны были быть автоматически созданы два файла: server.py и client.py. Если же этого не случилось по каким-то причинам, то просто нажимаем на проект, после чего высвечивается окно, в котором нажимаем на ```New```, а уже в новом окне ```Python file```. Таких нужно два - один для сервера, а другой для клиента соотвественно.
