...
* далее откроется новое окно, в котором нужно указать название проекта и его расположение. Далее на скринах проект будет называться ```Redis_project```. После указания всех настроек на панели слева появятся файлы проекта;
