...
Более подробная информация о том, какая строка за что отвечает, находится в файле Client.py.
```
...
Следующие методы почти полностью повторяет те же методы, что и на сервере - как минимум, выполняют те же действия, потому объяснения будут опущены.
```
    def get(self, key):
