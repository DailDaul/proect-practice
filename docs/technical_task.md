...
telnet 127.0.0.1
```
```Ip```, указанный в команде, это тот же самый ```ip-адрес```, который мы установили в методе ```_init_```. Если вы установили другой, то, соотвественно, в команде должен быть тот же ```ip```, который вы установили самостоятельно.
...
![](https://github.com/DailDaul/proect-practice/tree/master/src/screens)
